...
PROJECT=/root/EviAgri-VL/task14b_code_20260727_protocol_v1
...
TASK14A=/root/autodl-tmp/EviAgriDiag/experiments/task14a_full_frame_token_oracle/2026-07-27/protocol_v1/attempt_01/protocol/manifest.jsonl
ROOT=/root/autodl-tmp/EviAgriDiag/experiments/task14b_independent_token_oracle/2026-07-27/protocol_v1/attempt_01
...
test "$(sha256sum "$TASK14A" | cut -d' ' -f1)" = e691ce2735fa22d28c5fd1107fd6d00b9b368f7bf3f142782b54a42e50225199
...
"$PY_BASE" "$PROJECT/build_task14a_oracle_protocol.py" \
  ...
  --prior-positive "$TASK10/manifest.jsonl" --prior-positive "$TASK14A" \
  --plantseg "$PLANTSEG" --prior-used "$TASK11C" --prior-used "$TASK12A" \
  --prior-used "$TASK12B" --prior-used "$TASK13A" --prior-used "$TASK14A" \
  --locked-exclusion "$LOCKED" --train-per-class 4 --val-per-class 0 \
  --test-per-class 2 --null-count 32 --protocol-label task14b \
  --output-root "$ROOT/protocol"
(cd "$ROOT/protocol" && sha256sum -c completion.sha256)

"$PY_BASE" - "$ROOT/protocol/protocol_report.json" <<'PY'
import json, sys
report=json.load(open(sys.argv[1],encoding="utf-8"))
if report.get("status")!="PASSED_PROTOCOL" or report.get("row_count")!=128:
    raise SystemExit("Task14B metadata gate did not pass")
if report.get("rows_by_split")!={"probe_train":64,"probe_val":0,"probe_test":32,"null_test":32}:
    raise SystemExit("Task14B split gate did not pass")
if report.get("cross_split_component_overlap")!=0 or report.get("task8_locked_content_read") is not False:
    raise SystemExit("Task14B isolation gate did not pass")
PY

"$PY_VISION" "$PROJECT/extract_task14a_oracle_tokens.py" \
  --manifest "$ROOT/protocol/manifest.jsonl" --model-path "$MODEL" \
  --output-root "$ROOT/features" --expected-count 128 \
  --config-version task14b-oracle-feature-config-1 \
  --summary-version task14b-oracle-feature-summary-1
