...
相对 C1 的唯一配置干预是每个 seed 的训练步数由 8 增加到 64，并在固定步数保存观察点。训练仍使用 C1 完全相同的 64-row `smoke_train.jsonl`，不得切换到完整 192-row train；以下内容全部复用 C0 冻结协议：
...
每个 seed 必须从同一 Base 独立初始化；禁止续跑或加载 C1 adapter。64 steps 对应恰好 512 次样本 exposure，即同一 64-row smoke-train 的 8 次确定性暴露。cosine scheduler 类型、`learning_rate=1e-4` 和 `warmup_ratio=0.03` 不变，但 scheduler horizon 随 `max_steps=64` 派生，因此 C2 的 step 8 只作为 C2 内部学习曲线起点，不与 C1 的独立 8-step schedule 做数值趋势拼接。不得按 val、dev 或中间性能选择 checkpoint、seed 或超参数。

## 3. 数据边界

- 只读取 C0 已冻结的 train、val、dev 和由其确定的 smoke-dev；不重新抽样主 split。
- 只有从 train 冻结出的同一 64-row smoke-train 参与参数更新；其余 128 条 train 不参与 C2。val 只检查管线完整性；dev 只在设计与代码冻结后评测。
...
- C2 只覆盖 16 类、64 张训练图和 80 张 dev；即使科学 PASS，也不能外推到完整 IP102、AGE 或真实开放世界诊断。
- 64-row smoke-train 被重复暴露 8 次，存在记忆化风险；结论必须以 source/component 均互斥的 dev 表现和 image/no-image 配对差异为准，train loss 下降不能单独作为学习证据。
- 每类 dev 只有 5 张，head/medium/tail 与逐类指标不稳定；最终必须同时报告 1000 次 source-image bootstrap 95% CI，不能只引用点估计。
- 三个 seed 只用于观察稳定性，不足以证明广泛的优化分布规律；不得只展示最佳 seed。
- conditional log-likelihood Top-k 与格式法医指标只解释生成失败原因，不得替代严格生成式主指标或降低科学 PASS 标准。
- C2 的 checkpoint 共享同一连续训练轨迹，彼此并非独立实验；checkpoint 趋势只作描述性证据，不执行 checkpoint 间显著性检验。
