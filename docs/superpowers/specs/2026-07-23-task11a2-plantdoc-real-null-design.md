...
- evaluation_v1 观测到 40/40 拒答后，发现普通非参数 bootstrap 对全零事件退化为 `[0,0]`，不能表达有限样本不确定性；v1 原样保留。
- evaluation_v2 在原 gate 之外新增按40个唯一图像计算的 Clopper–Pearson 95% 区间及更保守的 upper `<25%` gate；三个 seed 不作为120个独立样本重复计数。
- 该补充不重选样本、模型、温度或阈值，只修正不确定性报告。
